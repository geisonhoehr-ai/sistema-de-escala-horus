...
export const EditUnavailabilityDialog = ({
  unavailability,
  trigger,
}: EditUnavailabilityDialogProps) => {
  const [open, setOpen] = useState(false)
  const { updateUnavailability, unavailabilityTypes } = useDataStore()
  const { toast } = useToast()

  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
    defaultValues: {
      unavailabilityTypeId: unavailability.unavailabilityTypeId,
      dateRange: {
        from: unavailability.startDate,
        to: unavailability.endDate,
      },
      reasonDetails: unavailability.reasonDetails || '',
    },
  })
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      toast({ title: 'Unavailability updated' })
      setOpen(false)
    } catch (error) {
      toast({
        title: 'Error',
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Update the unavailability details.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
